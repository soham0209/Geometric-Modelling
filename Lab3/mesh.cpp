...
void Mesh::AddFacet(vector<GeomVert> geomfacet) {
	...
	for (i = 0; i < mTopoFacets[ facet_ind ].GetNumberEdges(); i++) {		
        ...
        for (size_t j = 0; j < edge.GetNumberIncFacets(); j++) {
            if (edge.GetIncFacet((int)j) != facet_ind) {
                mTopoFacets[ facet_ind ].AddIncFacet( edge.GetIncFacet((int)j) );
                mTopoFacets[ edge.GetIncFacet((int)j) ].AddIncFacet( facet_ind );
			}
		}
	}
}
...
int Mesh::FindGeomVertex(GeomVert v) {
    for (size_t i= 0; i < mGeomVerts.size(); i++) {
        if (mGeomVerts[i] == v) return (int)i;
	}
	return -1;
}
// ------------------------------------------------------------
...
int	Mesh::FindTopoEdge(TopoEdge e) {
    for (size_t i = 0; i < mTopoEdges.size(); i++) {
        if (mTopoEdges[i] == e) return (int)i;
	}
	return -1;
}
// ------------------------------------------------------------
